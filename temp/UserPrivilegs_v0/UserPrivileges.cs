...
namespace MySqlClientDotNET.DesignControls.UserPrivilegesNS {
    public partial class UserPrivileges : DynamicTabNS.DynamicTabPage {
        ...
        public UserPrivileges() {
            ...
            hcheckAll = false;
            cntItem = 0;
            checkListDataG.ItemCheck += new System.Windows.Forms.ItemCheckEventHandler(this._ItemCheck);
            checkListStructG.ItemCheck += new ItemCheckEventHandler(this._ItemCheck);
            checkListAdminG.ItemCheck += new ItemCheckEventHandler(this._ItemCheck);
        }
...
FROM
    user";
        ...
        private void editPrivilegesToolStripMenuItem_Click(object sender, EventArgs e) {
            loadUserPrivG(gridTable.SelectedRows[0].Cells[0].Value.ToString(),
        ...
        private void loadUserPrivG(string user, string host) {
            isCreat = false;
            string query = 
@"SELECT 
    Select_priv,
...
AND 
    Host='" + host + "'";
                        ...
                        using (dr = cmmd.ExecuteReader()) {
                            ...
                            while (dr.Read()) {
                                for (int i = 0; i < 28; ++i) {
                                    ...
                                    if (i >= 0 && i < 5) {
                                        checkListDataG.SetItemChecked(cnt1, res.Equals("Y"));
                                        if (res.Equals("Y")) {
                                            ++_cnt1;
                                            ++cntItem;
                                        }
                                    ...
                                    if (i >= 5 && i < 17) {
                                        checkListStructG.SetItemChecked(cnt2, res.Equals("Y"));
                                        if (res.Equals("Y")) {
                                            ++_cnt2;
                                            ++cntItem;
                                        }
                                    ...
                                    if (i >= 17) {
                                        checkListAdminG.SetItemChecked(cnt3, res.Equals("Y"));
                                        if (res.Equals("Y")) {
                                            ++_cnt3;
                                            ++cntItem;
                                        }
                                ...
                                if (cnt1 == _cnt1)
                                    checkDataG.Checked = true;
                                else
                                    checkDataG.Checked = false;

                                if (cnt2 == _cnt2)
                                    checkStructG.Checked = true;
                                else
                                    checkStructG.Checked = false;

                                if (cnt3 == _cnt3)
                                    checkAdminG.Checked = true;
                                else
                                    checkAdminG.Checked = false;
        ...
        private void loadUserPrivD(string user, string host) {
            string query =
@"SELECT 
    Select_priv,
    Insert_priv,
    ...
    Host='" + host + "'";
        ...
        private void buttonApply_Click(object sender, EventArgs e) {
            ...
            if (cntItem == 28) {
                ...
                string data = "";
                for (int i = 0; i < checkListDataG.Items.Count; ++i) {
                    if (!checkListDataG.GetItemChecked(i))
                        continue;
                    data += ", " + checkListDataG.Items[i].ToString();
                }

                for (int i = 0; i < checkListStructG.Items.Count; ++i) {
                    if (!checkListStructG.GetItemChecked(i))
                        continue;
                    data += ", " + checkListStructG.Items[i].ToString();
                }

                for (int i = 0; i < checkListAdminG.Items.Count; ++i) {
                    if (!checkListAdminG.GetItemChecked(i))
                        continue;
                    if (i == 0)
                        data += ", GRANT OPTION";
                    else
                        data += ", " + checkListAdminG.Items[i].ToString();
                }
                if (!data.Equals(""))
        ...
        private void checkAllG(bool itIs) {
            checkDataG.Checked = itIs;
            checkStructG.Checked = itIs;
            checkAdminG.Checked = itIs;
        }
        ...
        private void _ItemCheck(object sender, ItemCheckEventArgs e) {
            ...
            try {
                if (((CheckedListBox)sender).Name.Equals("checkListDataG")) {
                    int cnt = 0;
                    for (int i = 0; i < checkListDataG.Items.Count; ++i) {
                        if (checkListDataG.GetItemChecked(i))
                            ++cnt;
                    ...
                    if (checkListDataG.Items.Count == cnt) {
                        hcheckAll = true;
                        checkDataG.Checked = true;
                        hcheckAll = false;
                    } else {
                ...
                } else if (((CheckedListBox)sender).Name.Equals("checkListStructG")) {
                    int cnt = 0;
                    for (int i = 0; i < checkListStructG.Items.Count; ++i) {
                        if (checkListStructG.GetItemChecked(i))
                            ++cnt;
                    ...
                    cnt += e.NewValue == CheckState.Checked ? 1 : -1;
                    if (checkListStructG.Items.Count == cnt) {
                        hcheckAll = true;
                        checkStructG.Checked = true;
                        hcheckAll = false;
                    } else {
                ...
                } else if (((CheckedListBox)sender).Name.Equals("checkListAdminG")) {
                    int cnt = 0;
                    for (int i = 0; i < checkListAdminG.Items.Count; ++i) {
                        if (checkListAdminG.GetItemChecked(i))
                            ++cnt;
                    ...
                    cnt += e.NewValue == CheckState.Checked ? 1 : -1;
                    if (checkListAdminG.Items.Count == cnt) {
                        hcheckAll = true;
                        checkAdminG.Checked = true;
                        hcheckAll = false;
                    } else {
        ...
        private void checkDataG_CheckedChanged(object sender, EventArgs e) {
            if (hcheckAll)
                return;
            hItemCheck = true;
            for (int i = 0; i < checkListDataG.Items.Count; ++i) {
                if (checkDataG.Checked) {
                    if (!checkListDataG.GetItemChecked(i))
                        ++cntItem;
                } else {
                    if (checkListDataG.GetItemChecked(i))
                ...
                checkListDataG.SetItemChecked(i, checkDataG.Checked);
            }
            hItemCheck = false;
        }

        private void checkStructG_CheckedChanged(object sender, EventArgs e) {
            if (hcheckAll)
                return;
            hItemCheck = true;
            for (int i = 0; i < checkListStructG.Items.Count; ++i) {
                if (checkStructG.Checked) {
                    if (!checkListStructG.GetItemChecked(i))
                        ++cntItem;
                } else {
                    if (checkListStructG.GetItemChecked(i))
                ...
                checkListStructG.SetItemChecked(i, checkStructG.Checked);
            }
            hItemCheck = false;
        }

        private void checkAdminG_CheckedChanged(object sender, EventArgs e) {
            if (hcheckAll)
                return;
            hItemCheck = true;
            for (int i = 0; i < checkListAdminG.Items.Count; ++i) {
                if (checkAdminG.Checked) {
                    if (!checkListAdminG.GetItemChecked(i))
                        ++cntItem;
                } else {
                    if (checkListAdminG.GetItemChecked(i))
                ...
                checkListAdminG.SetItemChecked(i, checkAdminG.Checked);
            }
            hItemCheck = false;
        }
        #endregion
        ...
        private void tabPrivilegs_SelectedIndexChanged(object sender, EventArgs e) {
            if (tabPrivilegs.SelectedIndex == 0) {
                numMC.Visible = true;
                numMQ.Visible = true;
                numMU.Visible = true;
                numMUC.Visible = true;
