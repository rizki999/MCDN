...
namespace MySql.Data.MySqlClient
{
    public class MySqlTriggerList : IDisposable
    {
        ...
        public MySqlTriggerList(MySqlCommand cmd, string db)
        {
            _sqlShowTriggers = string.Format("SHOW TRIGGERS FROM {0};", db);
            try
            {
                ...
                {
                    _lst.Add(new MySqlTrigger(cmd, db + "`.`" + dr["Trigger"] + "", dr["Definer"] + ""));
                }
            }
