...
namespace MySql.Data.MySqlClient
{
    public class MySqlTableList : IDisposable
    {
        ...
        public MySqlTableList(MySqlCommand cmd, string db)
        {
            _sqlShowFullTables = " SELECT TABLE_NAME, TABLE_TYPE FROM information_schema.TABLES WHERE TABLE_SCHEMA='" + db + "';";
            DataTable dtTableList = QueryExpress.GetTable(cmd, _sqlShowFullTables);

            foreach (DataRow dr in dtTableList.Rows)
            {
                _lst.Add(new MySqlTable(cmd, dr[0].ToString(), db));
            }
        }
