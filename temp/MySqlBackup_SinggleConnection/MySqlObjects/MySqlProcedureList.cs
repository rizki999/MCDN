...
namespace MySql.Data.MySqlClient
{
    public class MySqlProcedureList : IDisposable
    {
        ...
        public MySqlProcedureList(MySqlCommand cmd, string db)
        {
            try
            {
                string dbname = db;//QueryExpress.ExecuteScalarStr(cmd, "SELECT DATABASE();");
                ...
                {
                    _lst.Add(new MySqlProcedure(cmd, dbname + "`.`" + dr["Name"] + "", dr["Definer"] + ""));
                }
            }
