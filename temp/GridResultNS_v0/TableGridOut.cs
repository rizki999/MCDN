...
namespace MySqlClientDotNET.DesignControls.GridResultNS {
    public partial class GridResult : DynamicTabPage {
        ...
        public GridResult(OutGrid oGrid) {
            ...
            checkBoxIgnorePri.Checked = true;
        ...
        public int IndexPrimaryKey { get; set; }
        ...
        private void stripButtonDel_Click(object sender, EventArgs e) {
            ...
            if (outGrid == OutGrid.ExecutedFromSQLBuilder) {
                ...
                for (int o = 0; o < selDelCount; ++o) {
                    ...
                    for (int p = 0; p < listEditRow.Count; ++p) {
                        if (gridTable.SelectedRows[o].Index == listEditRow[p]) {
                            listDelRow.Add(listEditRowPri[p]);
                            dup = true;
                    ...
                    if (!dup)
                        listDelRow.Add(gridTable.SelectedRows[o].Cells[IndexPrimaryKey].Value.ToString());
                    else
                        dup = false;
                }
                ...
                if ((listAddRow.Count + listEditRow.Count + listDelRow.Count) > 0)
                    this.IsEdited = true;
                else
                    this.IsEdited = false;
            }
        ...
        private string sqlBuilder() {
            ...
            for (int i = 0; i < listDelRow.Count; ++i) {
                val = setValue(listDelRow[i], IndexPrimaryKey);
                res += "DELETE FROM `" + TableName + "` WHERE `" + dataTable.Columns[IndexPrimaryKey].ColumnName + "`=" + val + ";\n";
            }
            ...
            for (int i = 0; i < listEditRow.Count; ++i) {
                ...
                if (!colName.Equals("")) {
                    ...
                    val = setValue(listEditRowPri[i], IndexPrimaryKey);
                    update += colName + " WHERE `" + dataTable.Columns[IndexPrimaryKey].ColumnName + "`=" + val + ";\n";
                } else {
                    update = string.Empty;
                }
            }
        ...
        private void buttonApply_Click(object sender, EventArgs e) {
            if (outGrid == OutGrid.ExecutedFromSQLBuilder) {
                int isChanged = listAddRow.Count + listEditRow.Count + listDelRow.Count;
                if (!this.IsEdited) {
        ...
        private void gridTable_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e) {
            if (outGrid == OutGrid.ExecutedFromSQLBuilder) {
                listEditRowPri.Add(gridTable[IndexPrimaryKey, e.RowIndex].Value.ToString());
                isEditedCell = false;
            }
        }
